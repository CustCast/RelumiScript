...
namespace RelumiScript
{
    public partial class MainWindow : Avalonia.Controls.Window
    {
        ...
        private string GetThemesFolder()
        {
            string baseDir = AppDomain.CurrentDomain.BaseDirectory;
            string themesDir = Path.Combine(baseDir, "Themes");
        ...
        private void LoadThemeByName(string name)
        {
            if (string.IsNullOrEmpty(name)) return;
            try
            {
                string path = Path.Combine(GetThemesFolder(), name + ".json");
                if (File.Exists(path))
                {
                    var settings = JsonConvert.DeserializeObject<ThemeSettings>(File.ReadAllText(path));
                    if (settings != null) InjectTheme(settings);
                }
            }
            catch (Exception ex) { StatusText.Text = $"Error loading theme {name}: {ex.Message}"; }
        }

        private void SaveThemeByName(string name)
        {
            if (string.IsNullOrEmpty(name)) return;
            try
            {
                ...
                StatusText.Text = $"Theme '{name}' saved.";
            }
            catch (Exception ex) { StatusText.Text = $"Error saving theme: {ex.Message}"; }
        }
        ...
        {
            if (!string.IsNullOrEmpty(_themeVm.CurrentThemeName))
            {
                SaveThemeByName(_themeVm.CurrentThemeName);
                await InjectTheme(null);
            }
        }

        public void BtnResetTheme_Click(object? sender, RoutedEventArgs e)
        {
            if (!string.IsNullOrEmpty(_themeVm.CurrentThemeName))
            {
                LoadThemeByName(_themeVm.CurrentThemeName);
                StatusText.Text = "Theme changes discarded.";
            }
        }

        private async Task InjectTheme(ThemeSettings? directSettings = null)
        {
            ThemeSettings settings = directSettings ?? _themeVm.ToSettings();
            _themeVm.LoadFromSettings(settings);
            UpdateDynamicResources(); // Force UI update
        ...
        private async void OpenDocument(object node, bool isPeek)
        {
            ...
            else if (node is ScriptNode sNode) { docId = "SCRIPT:" + sNode.Label; title = sNode.Label; content = sNode.Content; }
            else return;
            ...
            if (isPeek)
            {
                ...
                if (currentPreview != null) { if (currentPreview.IsDirty) { currentPreview.IsPreview = false; _documents.Add(newDoc); } else { int index = _documents.IndexOf(currentPreview); _documents[index] = newDoc; } }
                else _documents.Add(newDoc);
            }
            else _documents.Add(newDoc);
        ...
        {
            if (sender is Button btn && btn.Tag is EditorDocument doc) CloseDocument(doc);
        }

        private async void CloseDocument(EditorDocument doc)
        {
            if (doc.IsDirty) { var dialog = new DiscardChangesDialog(doc.Title.TrimEnd('*')); var result = await dialog.ShowDialog<bool>(this); if (result) ForceCloseDocument(doc); }
            else ForceCloseDocument(doc);
        }
        ...
        private void ForceCloseDocument(EditorDocument doc)
        {
            ...
            if (_documents.Count == 0) { _activeDocument = null; SetEditorText(""); }
            else if (_activeDocument == doc) { int newIndex = Math.Max(0, index - 1); if (newIndex < _documents.Count) SetActiveDocument(_documents[newIndex]); }
            UpdateNoTabsPlaceholder();
        }
        ...
        private void OnEditorMessageReceived(object? sender, WebViewMessageReceivedEventArgs e)
        {
            ...
            if (e.Message.StartsWith("CONTENT_UPDATE:")) { string newContent = e.Message.Substring(15); if (_activeDocument != null) { if (_activeDocument.IsPreview) _activeDocument.IsPreview = false; _activeDocument.Content = newContent; _activeDocument.IsDirty = !string.Equals(_activeDocument.Content, _activeDocument.OriginalContent); } }
        }
        ...
        public async void BtnLoad_Click(object? sender, RoutedEventArgs e)
        {
            ...
            try
            {
                ...
                StatusText.Text = "Loading Game Data..."; await Task.Run(() => _service.LoadGameData(root));
                StatusText.Text = "Loading Scripts..."; var scripts = await Task.Run(() => _service.LoadAndDecompile(root));
                StatusText.Text = "Loading Messages..."; _loadedMessages = await Task.Run(() => _service.LoadMessageFiles(root));
        ...
        public async void JumpToLocation_Click(object? sender, RoutedEventArgs e)
        {
            ...
            {
                _isNavigating = true;
                try { if (loc.NodeObject != null) OpenDocument(loc.NodeObject, isPeek: true); if (_isEditorReady) { await Task.Delay(50); await Editor.ExecuteScriptAsync($"editor.revealLineInCenter({loc.LineNumber}); editor.setPosition({{lineNumber: {loc.LineNumber}, column: 1}}); editor.focus();"); } }
                finally { _isNavigating = false; }
            }
        }

        // -----------------------------------------------------------------------------
        // SEARCH, BOTTOM NAV, UTILS
        // -----------------------------------------------------------------------------
        ...
        private void SwitchBottomView(string viewName)
        {
            ...
            BottomPanelContainer.IsVisible = true; _currentBottomView = viewName;
            TerminalPanel.IsVisible = viewName == "Terminal"; PreviewPanel.IsVisible = viewName == "Preview";
        ...
        private async void RefreshAllTrackers()
        {
            ...
            try
            {
                List<object> nodesToScan = new List<object>(); if (ScriptTree.ItemsSource is System.Collections.IEnumerable items) { foreach (var item in items) nodesToScan.Add(item); }
                LoadWorkIdMap();
                var result = await ScriptScanner.ScanAllAsync(nodesToScan, _service.FlagMap, _service.SysFlagMap, _workIdMap);
                _allFlagUsages = result.Flags; _allWorkUsages = result.Works; _allCommandUsages = result.Commands; _allEventUsages = result.Events;
                FilterFlags(FlagSearchBox.Text ?? ""); FilterWorks(ScriptSearchBox.Text ?? "");
                StatusText.Text = $"Scanner: Indexed {result.Flags.Count} flags, {result.Works.Count} works.";
            }
            catch (Exception ex) { StatusText.Text = $"Scanner Error: {ex.Message}"; }
        }
        ...
        {
            _workIdMap.Clear(); string? jsonDir = FindJsonFolder(); if (string.IsNullOrEmpty(jsonDir)) return;
            try { string workPath = Path.Combine(jsonDir, "work.json"); if (File.Exists(workPath)) { var jArray = JArray.Parse(File.ReadAllText(workPath)); foreach (var item in jArray) { int id = item["Id"]?.Value<int>() ?? 0; string name = item["Name"]?.Value<string>() ?? ""; if (!_workIdMap.ContainsKey(id)) _workIdMap.Add(id, name); } } } catch { }
        }
        ...
        {
            TryInitMessageRenderer(); if (_messageRenderer == null) return;
        ...
        private async void TerminalInput_KeyDown(object? sender, KeyEventArgs e)
        {
            ...
            string cmd = TerminalInput.Text; TerminalOutput.Text += $"> {cmd}\n"; TerminalInput.Text = "";
            ...
            await Task.Run(() => {
                try
                {
                    ...
                    using var p = Process.Start(info); if (p == null) return; string o = p.StandardOutput.ReadToEnd(); string err = p.StandardError.ReadToEnd(); p.WaitForExit();
